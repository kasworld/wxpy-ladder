...
class LadderControl(wx.Control):
    ...
    def _OnPaint(self, evt):
        ...
        unitx, unity  = float(size[0]) / self.playernum , float(size[1]) / self.ladderlen
        if not self.showhint :
            dc.SetPen(wx.Pen("Black", 2))
            for x in range(self.playernum):
                dc.DrawLine( unitx*x + unitx/2 , 0, unitx*x + unitx/2,size[1])
        else :
            ...
            for y in range(self.ladderlen+1) :
                for x in range( self.playernum ):
                    ...
                    dc.DrawLine( x*unitx+unitx/2 , y*unity-unity/2, x*unitx+unitx/2 ,y*unity+unity/2)
                for x in range( self.playernum ):
            ...
            for y in range(self.ladderlen+1) :
                for x in range( self.playernum ):
                    if y < self.ladderlen and x < self.playernum-1 and  self.steps[y][x]:
                        ...
                        dc.DrawLine( x*unitx+unitx/2 , y*unity+unity/2-2, (x+1)*unitx+unitx/2 ,y*unity+unity/2-2)
    ...
    def play(self):
        ...
        for i in range(self.playernum):
            self.playercolors.append( (random.randint(0,255), random.randint(0,255),random.randint(0,255),) )
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.playernum = 10
        self.slider_1 = wx.Slider(self, -1, self.playernum, 2, self.maxplayernum, style=wx.SL_HORIZONTAL|wx.SL_LABELS)
